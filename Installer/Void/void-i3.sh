...
wget --tries=20 $dlink2/i3.sh -O $folder/root/i3.sh > /dev/null
clear
echo "Setting up the installation of Openbox VNC"

echo "#!/bin/bash
clear
 echo 'Installing i3... This might take some time depending upon your Internet'
xbps-install -Su -y > /dev/null 
xbps-install -S lxde tigervnc wget sudo -y 
clear
if [ ! -f /root/i3.sh ]; then
    wget --tries=20 $dlink2/i3.sh -O /root/i3.sh > /dev/null
    bash ~/i3.sh 
else
    bash ~/i3.sh
fi
...
if [ ! -f /usr/bin/vncserver ]; then
    xbps-install -S i3 tigervnc wget -y  > /dev/null
fi
