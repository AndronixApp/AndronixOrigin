...
wget --tries=20 $dlink/XFCE4/xfce4_de.sh -P $folder/root > /dev/null
clear
echo "Setting up the installation of XFCE VNC"

echo "#!/bin/bash
xbps-install -Su -y && xbps-install -S xfce4 tigervnc wget  -y 

if [ ! -f /root/lxde_de.sh ]; then
    wget --tries=20 $dlink/XFCE4/xfce4_de.sh -P /root > /dev/null
    bash ~/xfce4_de.sh 
else
    bash ~/xfce4_de.sh
fi
...
if [ ! -f /usr/local/bin/vncserver-start ]; then
    wget --tries=20  $dlink/XFCE4/vncserver-start > /dev/null
    wget --tries=20 $dlink/XFCE4/vncserver-stop > /dev/null
fi
