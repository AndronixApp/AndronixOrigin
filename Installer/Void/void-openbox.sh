...
wget --tries=20 $dlink2/openbox.sh -O $folder/root/openbox.sh > /dev/null
clear
echo "Setting up the installation of Openbox VNC"

echo "#!/bin/bash
clear
 echo 'Installing Openbox... This might take some time depending upon your Internet'
xbps-install -Su -y > /dev/null 
xbps-install -S lxde tigervnc wget sudo -y 
clear
if [ ! -f /root/openbox.sh ]; then
    wget --tries=20 $dlink2/openbox.sh -O /root/openbox.sh > /dev/null
    bash ~/openbox.sh 
else
    bash ~/openbox.sh
fi
...
if [ ! -f /usr/bin/vncserver ]; then
    xbps-install -S openbox tigervnc wget -y  > /dev/null
fi
