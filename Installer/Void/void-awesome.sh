...
wget --tries=20 $dlink2/awesome.sh -O $folder/root/awesome.sh > /dev/null
clear
...
if [ ! -f /root/awesome.sh ]; then
    wget --tries=20 $dlink2/awesome.sh -O /root/awesome.sh > /dev/null
    bash ~/awesome.sh 
else
    bash ~/awesome.sh
fi
...
if [ ! -f /usr/bin/vncserver ]; then
    xbps-install -S awesome tigervnc wget -y  > /dev/null
fi
